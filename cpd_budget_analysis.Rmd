...
[GitHub Repository](https://github.com/rwrcrump/cpd_budget_analysis/tree/main)
...
```{r salary appropriations by dept bar chart}
ts_FY21 %>% 
  group_by(department_description) %>% 
  summarize(payroll_app = sum(total_budgeted_amount)) %>% 
  ggplot() +
  geom_col(aes(
    fct_reorder(department_description, payroll_app), 
    payroll_app / 1000000, 
    fill = payroll_app)
    ) +
  labs(title = "Payroll Appropriations by Department",
...
FY21 %>% 
  ...
  # replace to account for total pension cost
  mutate(appropriation_account_description = if_else(appropriation_account_description == "LOSS IN COLLECTION OF TAXES","EMPLOYEE ANNUITY AND BENEFIT", appropriation_account_description)) %>% 
  group_by(appropriation_account_description) %>% 
